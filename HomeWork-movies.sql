...
SELECT `title`,`rating` AS 'IMDb Score' FROM `movies` WHERE `genre` IN ( 'Sci-Fi','Action');

SELECT `title`, `release_year` AS 'Release'FROM movies WHERE `release_year` BETWEEN 2008 AND 2015;

SELECT title,rating AS 'High Rated Movies' FROM movies GROUP BY rating HAVING rating > 8.5;
